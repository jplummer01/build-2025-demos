...
async def display_evaluation_summary(eval_ids: list):
    ...
    if all_eval_runs:
        ...
        df = df[['id', 'name', 'model', 'status', 'pass_percentage', 'error_percentage', 'abs_pass_percentage', 'eval_id']]  # Select relevant columns
        ...
        print("=" * 50)
        print(df.to_string(index=False, header=["Run ID", "Run Name", "Model", "Status", "Pass Percentage (%)", "Error Percentage (%)", "Abs Pass Percentage (%)", "Evaluation ID", "Evaluation Name"]))
        print("=" * 50)
        ...
        # Plot absolute pass percentage
        plt.barh(
            df['display_label'], 
            df['abs_pass_percentage'], 
            color='gray', 
            edgecolor='black', 
            alpha=0.5, 
            label='Absolute Pass Percentage (%)'
        )

        plt.xlabel('Percentage (%)')
        plt.ylabel('Model')
        plt.title("Pass Percentage and Absolute Pass Percentage by Model Across Evaluations")
